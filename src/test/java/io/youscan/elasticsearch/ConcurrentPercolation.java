...
public class ConcurrentPercolation extends AbstractNodesTests {
    ...
    public void testSimpleConcurrentPercolator() throws Exception {
        ...
        final PercolateSourceBuilder onlyField1 = new PercolateSourceBuilder().setDoc(new PercolateSourceBuilder.DocBuilder().setDoc(
                jsonBuilder()
                        .startObject()
                        .field("field1", 1)
                        .endObject()
        ));
        final PercolateSourceBuilder onlyField2 = new PercolateSourceBuilder().setDoc(new PercolateSourceBuilder.DocBuilder().setDoc(
                jsonBuilder()
                        .startObject()
                        .field("field2", "value")
                        .endObject()
        ));
        final PercolateSourceBuilder bothFields = new PercolateSourceBuilder().setDoc(new PercolateSourceBuilder.DocBuilder().setDoc(
                jsonBuilder()
                        .startObject()
                        .field("field1", 1)
                        .field("field2", "value")
        ...
        client.prepareIndex("index", YPercolatorService.TYPE_NAME, "test1")
                .setSource(XContentFactory.jsonBuilder().startObject().field("query", termQuery("field2", "value")).endObject())
                .execute().actionGet();
        client.prepareIndex("index", YPercolatorService.TYPE_NAME, "test2")
        ...
        for (int i = 0; i < threads.length; i++) {
            Runnable r = percolationRunnable(
                    onlyField1,
                    onlyField2,
                    bothFields,
                    start,
    ...
    public void testConcurrentAddingAndRemovingWhilePercolating() throws Exception {
        ...
        PercolateSourceBuilder source = new PercolateSourceBuilder().setDoc(
                new PercolateSourceBuilder.DocBuilder().setDoc(jsonBuilder()
                        .startObject()
                        .field("field1", "value")
                        .endObject())
        );
        ...
        for (int counter = 0; counter < numberPercolateOperation; counter++) {
            ...
            try {
                ...
                YPercolateResponse response = new YPercolateRequestBuilder(client).setIndices("index").setDocumentType("type")
    ...
    private Runnable percolationRunnable(final PercolateSourceBuilder onlyField1,
                                         final PercolateSourceBuilder onlyField2,
                                         final PercolateSourceBuilder bothFields,
            ...
            public void run() {
                try {
                    ...
                    while (!stop.get()) {
                        ...
                        YPercolateResponse percolate;
                        if (count % 3 == 0) {
                            percolate = new YPercolateRequestBuilder(client).setIndices("index").setDocumentType("type")
                    ...
                    while (run.get()) {
                        ...
                        try {
                            if (!liveIds.isEmpty() && random.nextInt(100) < 19) {
                                ...
                                DeleteResponse response = client.prepareDelete("index", YPercolatorService.TYPE_NAME, id)
                                        .execute().actionGet();
                                ...
                                IndexResponse response = client.prepareIndex("index", YPercolatorService.TYPE_NAME, id)
                                        .setSource(doc)
                                        .execute().actionGet();
                                liveIds.add(id);
